...
export default function FinanceMachineMaster() {
  ...
  const handleSync = async () => {
    setSyncing(true);
    const toastId = toast.loading("Syncing finance machine data from external API...");
    try {
      const response = await syncMasterData({ syncBrands: false, syncCompanies: false, syncMachines: true });
      if (response.data?.success) {
        const addedCount = response.data.data?.financeMachines?.added || 0;
        const skippedCount = response.data.data?.financeMachines?.skipped || 0;
      ...
      console.error("Failed to sync finance machine data", err);
      toast.error(err?.response?.data?.message || "Unable to sync finance machine data. Please try again.", { id: toastId });
    } finally {
      setSyncing(false);
    }
  };
          ...
          actionButton={
            hasPermission("finance_machine_master", "write") ? (
              <button
                ...
                title="Sync Finance Machine Data"
              >
