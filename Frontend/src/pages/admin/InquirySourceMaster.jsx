...
// ─── Inquiry Source Modal (Handles both Create and Edit) ───────────────────────────────────
function InquirySourceModal({ isOpen, row, onClose, onSave, saving }) {
  ...
  const handleSubmit = (e) => {
    ...
    onSave(isEdit ? row.id : null, sourceName.trim());
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "20px 28px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(135deg,#6804a1,#52037e)" }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#fff" }}>{isEdit ? "Edit Inquiry Source" : "Create Inquiry Source"}</h2>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#d9e2ec" }}>{isEdit ? "Update inquiry source name" : "Add a new inquiry source to the system"}</p>
          </div>
          ...
          <div style={{ padding: "24px 28px" }}>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>
                Source Name <span style={{ color: "#e11d48" }}>*</span>
              </label>
              <input
                type="text"
                value={sourceName}
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function InquirySourceMaster() {
  ...
  const loadSources = async () => {
    ...
    try {
      ...
      setError("Unable to load inquiry sources. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (id, sourceName) => {
    setSaving(true);
    try {
      if (id) {
        // Edit Mode
        await updateInquirySource(id, { sourceName });
        toast.success("Inquiry source updated successfully");
      } else {
        // Create Mode
        await createInquirySource({ sourceName });
      ...
      console.error("Failed to save inquiry source", err);
      toast.error(err?.response?.data?.message || "Unable to save inquiry source. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this inquiry source?")) return;
    setSaving(true);
    try {
      await deleteInquirySource(id);
      toast.success("Inquiry source deleted successfully");
      await loadSources();
    } catch (err) {
      console.error("Failed to delete inquiry source", err);
      toast.error(err?.response?.data?.message || "Unable to delete inquiry source.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns = useMemo(() => {
    const cols = [
      { key: "id", label: "ID", minWidth: "80px" },
      {
        ...
        render: (row) => <span style={{ fontWeight: 700, color: "#0f172a" }}>{row.source_name}</span>
      }
    ];

    const canUpdate = hasPermission("inquiry_source_master", "update");
    const canDelete = hasPermission("inquiry_source_master", "delete");
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, background: "#f8fafc", fontFamily: "'Inter',sans-serif" }}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", width: "100%", margin: "0 auto", padding: "32px 30px" }}>
        ...
        <DataTable
          ...
          actionButton={
            hasPermission("inquiry_source_master", "write") ? (
              <button
                onClick={() => {
                  setSelectedRow(null);
                  setIsModalOpen(true);
