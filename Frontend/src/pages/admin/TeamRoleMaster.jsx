...
import { getTeamRoles, createTeamRole, updateTeamRole, deleteTeamRole } from "../../api/teamRoleApi";
import DataTable from "../../components/DataTable";
import toast from "react-hot-toast";
import { usePermission } from "../../context/PermissionContext";

// ─── Team Role Modal (Handles both Create and Edit) ───────────────────────────────────
function TeamRoleModal({ isOpen, row, onClose, onSave, saving }) {
  ...
  useEffect(() => {
    if (row) {
      setRole(row.role || "");
      setIsRequired(row.is_required === 1 || row.is_required === true);
    } else {
      setRole("");
      setIsRequired(false);
    }
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!role.trim()) return;
    onSave(isEdit ? row.id : null, role.trim(), isRequired);
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "20px 28px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(135deg,#6804a1,#52037e)" }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#fff" }}>{isEdit ? "Edit Team Role" : "Create Team Role"}</h2>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#d9e2ec" }}>{isEdit ? "Update team role details" : "Add a new team role to the system"}</p>
          </div>
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function TeamRoleMaster() {
  ...
  const loadRoles = async () => {
    ...
    try {
      ...
      setError("Unable to load team roles. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (id, role, isRequired) => {
    setSaving(true);
    try {
      if (id) {
        // Edit Mode
        await updateTeamRole(id, { role, isRequired });
        toast.success("Team role updated successfully");
      } else {
        // Create Mode
        await createTeamRole({ role, isRequired });
        toast.success("Team role created successfully");
      }
      setIsModalOpen(false);
      setSelectedRow(null);
      await loadRoles();
    } catch (err) {
      console.error("Failed to save team role", err);
      toast.error(err?.response?.data?.message || "Unable to save team role. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || "Unable to delete team role.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns = useMemo(() => {
    const cols = [
      ...
      {
        key: "role", label: "Role",
        render: (row) => <span style={{ fontWeight: 700, color: "#0f172a" }}>{row.role}</span>
      },
      {
        ...
        render: (row) => (
          <span style={{
            ...
            fontSize: "12px",
            fontWeight: "700",
            background: row.is_required === 1 ? "#ecfdf5" : "#f1f5f9",
            color: row.is_required === 1 ? "#047857" : "#475569"
          }}>
            {row.is_required === 1 ? "Yes" : "No"}
          </span>
        )
      }
    ];

    const canUpdate = hasPermission("team_role_master", "update");
    const canDelete = hasPermission("team_role_master", "delete");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, background: "#f8fafc", fontFamily: "'Inter',sans-serif" }}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", width: "100%", margin: "0 auto", padding: "32px 30px" }}>
        ...
        <DataTable
          tableId="team_role_master"
          title="Team Role Master"
          data={roles}
          columns={columns}
          loading={loading}
          searchPlaceholder="Search team roles..."
          actionButton={
            hasPermission("team_role_master", "write") ? (
              <button
                onClick={() => {
                  setSelectedRow(null);
                  setIsModalOpen(true);
