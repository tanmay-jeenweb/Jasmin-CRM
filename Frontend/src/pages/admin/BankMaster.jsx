...
// ─── Bank Modal (Handles both Create and Edit) ───────────────────────────────────
function BankModal({ isOpen, row, onClose, onSave, saving }) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "20px 28px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(135deg,#6804a1,#52037e)" }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#fff" }}>{isEdit ? "Edit Bank/Card" : "Create Bank/Card"}</h2>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#d9e2ec" }}>{isEdit ? "Update bank or card name" : "Add a new bank or card to the system"}</p>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ padding: "24px 28px" }}>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>
                Bank / Card Name <span style={{ color: "#e11d48" }}>*</span>
              </label>
              <input
                type="text"
                value={bankCardName}
          ...
          <div style={{ padding: "16px 28px", borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "flex-end", gap: 12, background: "#fafafa" }}>
            ...
            <button
              ...
              {saving ? "Saving…" : isEdit ? "Save Changes" : "Create Bank/Card"}
            </button>
          </div>
        </form>
      </div>
...
export default function BankMaster() {
  ...
  const handleSave = async (id, bankCardName) => {
    ...
    try {
      if (id) {
        ...
        toast.success("Bank/Card created successfully");
      }
      ...
      toast.error(err?.response?.data?.message || "Unable to save bank/card. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || "Unable to delete bank/card.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns = useMemo(() => {
    const cols = [
      ...
      {
        key: "bank_card_name", label: "Bank/Card Name",
