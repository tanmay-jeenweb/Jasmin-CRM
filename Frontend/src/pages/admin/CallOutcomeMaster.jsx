...
import { getCallOutcomes, createCallOutcome, updateCallOutcome, deleteCallOutcome } from "../../api/callOutcomeApi";
import DataTable from "../../components/DataTable";
import toast from "react-hot-toast";
import { usePermission } from "../../context/PermissionContext";

// ─── Call Outcome Modal (Handles both Create and Edit) ───────────────────────────────────
function CallOutcomeModal({ isOpen, row, onClose, onSave, saving }) {
  const [outcomeName, setOutcomeName] = useState("");

  useEffect(() => {
    if (row) {
      setOutcomeName(row.outcome_name || "");
    } else {
      setOutcomeName("");
    }
  }, [row, isOpen]);
  ...
  const handleSubmit = (e) => {
    ...
    onSave(isEdit ? row.id : null, outcomeName.trim());
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "20px 28px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(135deg,#6804a1,#52037e)" }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#fff" }}>{isEdit ? "Edit Call Outcome" : "Create Call Outcome"}</h2>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#d9e2ec" }}>{isEdit ? "Update call outcome name" : "Add a new call outcome to the system"}</p>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ padding: "24px 28px" }}>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>
                Outcome Name <span style={{ color: "#e11d48" }}>*</span>
              </label>
              <input
                type="text"
                value={outcomeName}
                ...
                required
                placeholder="e.g. Connected, Busy, No Answer"
          ...
          <div style={{ padding: "16px 28px", borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "flex-end", gap: 12, background: "#fafafa" }}>
            ...
            <button
              type="submit"
              disabled={saving || !outcomeName.trim()}
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function CallOutcomeMaster() {
  ...
  const loadOutcomes = async () => {
    ...
    try {
      ...
      console.error("Failed to load call outcomes", err);
      setError("Unable to load call outcomes. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (id, outcomeName) => {
    setSaving(true);
    try {
      if (id) {
        // Edit Mode
        await updateCallOutcome(id, { outcomeName });
        toast.success("Call outcome updated successfully");
      } else {
        // Create Mode
        await createCallOutcome({ outcomeName });
        toast.success("Call outcome created successfully");
      }
      ...
      toast.error(err?.response?.data?.message || "Unable to save call outcome. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this call outcome?")) return;
    setSaving(true);
    try {
      await deleteCallOutcome(id);
      toast.success("Call outcome deleted successfully");
      await loadOutcomes();
    } catch (err) {
      console.error("Failed to delete call outcome", err);
      toast.error(err?.response?.data?.message || "Unable to delete call outcome.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns = useMemo(() => {
    const cols = [
      ...
      {
        key: "outcome_name", label: "Outcome Name",
        render: (row) => <span style={{ fontWeight: 700, color: "#0f172a" }}>{row.outcome_name}</span>
      }
    ];

    const canUpdate = hasPermission("call_outcome_master", "update");
    const canDelete = hasPermission("call_outcome_master", "delete");
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, background: "#f8fafc", fontFamily: "'Inter',sans-serif" }}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", width: "100%", margin: "0 auto", padding: "32px 30px" }}>
        ...
        <DataTable
          ...
          actionButton={
            hasPermission("call_outcome_master", "write") ? (
              <button
                onClick={() => {
                  setSelectedRow(null);
                  setIsModalOpen(true);
