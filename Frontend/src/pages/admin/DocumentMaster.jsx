...
import { getDocuments, createDocument, updateDocument, deleteDocument } from "../../api/documentApi";
import DataTable from "../../components/DataTable";
import toast from "react-hot-toast";
import { usePermission } from "../../context/PermissionContext";

// ─── Document Modal (Handles both Create and Edit) ───────────────────────────────────
function DocumentModal({ isOpen, row, onClose, onSave, saving }) {
  ...
  const handleSubmit = (e) => {
    ...
    onSave(isEdit ? row.id : null, docType.trim(), isRequired);
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "20px 28px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(135deg,#6804a1,#52037e)" }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#fff" }}>{isEdit ? "Edit Document Type" : "Create Document Type"}</h2>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#d9e2ec" }}>{isEdit ? "Update document type details" : "Add a new document type to the system"}</p>
          </div>
          ...
          <div style={{ padding: "24px 28px" }}>
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>
                Document Type <span style={{ color: "#e11d48" }}>*</span>
              </label>
              <input
                type="text"
                value={docType}
                ...
                required
                placeholder="e.g. GST Certificate, PAN Card, Aadhar Card"
...
export default function DocumentMaster() {
  ...
  const loadDocuments = async () => {
    ...
    try {
      ...
      setError("Unable to load document types. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (id, docType, isRequired) => {
    setSaving(true);
    try {
      if (id) {
        // Edit Mode
        await updateDocument(id, { docType, isRequired });
        toast.success("Document type updated successfully");
      } else {
        // Create Mode
        await createDocument({ docType, isRequired });
      ...
      toast.error(err?.response?.data?.message || "Unable to save document type. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || "Unable to delete document type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns = useMemo(() => {
    const cols = [
      { key: "id", label: "ID", minWidth: "80px" },
      {
        ...
        render: (row) => <span style={{ fontWeight: 700, color: "#0f172a" }}>{row.doc_type}</span>
      },
      {
    ...
    const canUpdate = hasPermission("document_master", "update");
    const canDelete = hasPermission("document_master", "delete");
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, background: "#f8fafc", fontFamily: "'Inter',sans-serif" }}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", width: "100%", margin: "0 auto", padding: "32px 30px" }}>
        ...
        <DataTable
          ...
          actionButton={
            hasPermission("document_master", "write") ? (
              <button
                onClick={() => {
                  setSelectedRow(null);
                  setIsModalOpen(true);
