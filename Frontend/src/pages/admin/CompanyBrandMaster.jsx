...
import { getCompanyBrands, createCompanyBrand, updateCompanyBrand, deleteCompanyBrand } from "../../api/companyBrandApi";
...
// ─── Company Brand Modal (Handles both Create and Edit) ───────────────────────────────────
function CompanyBrandModal({ isOpen, row, onClose, onSave, saving }) {
  ...
  const handleSubmit = (e) => {
    ...
    onSave(isEdit ? row.id : null, brandName.trim());
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "20px 28px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(135deg,#6804a1,#52037e)" }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#fff" }}>{isEdit ? "Edit Company Brand" : "Create Company Brand"}</h2>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#d9e2ec" }}>{isEdit ? "Update brand name" : "Add a new company brand to the system"}</p>
          </div>
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function CompanyBrandMaster() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (response.data?.success) {
        const addedCount = response.data.data?.companyBrands?.added || 0;
        const skippedCount = response.data.data?.companyBrands?.skipped || 0;
  ...
  const loadBrands = async () => {
    ...
    try {
      ...
      setError("Unable to load company brands. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (id, brandName) => {
    setSaving(true);
    try {
      if (id) {
        // Edit Mode
        await updateCompanyBrand(id, { brandName });
        toast.success("Company brand updated successfully");
      } else {
        // Create Mode
        await createCompanyBrand({ brandName });
        toast.success("Company brand created successfully");
      }
      ...
      toast.error(err?.response?.data?.message || "Unable to save company brand. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this company brand?")) return;
    setSaving(true);
    try {
      await deleteCompanyBrand(id);
      toast.success("Company brand deleted successfully");
      await loadBrands();
    } catch (err) {
      console.error("Failed to delete company brand", err);
      toast.error(err?.response?.data?.message || "Unable to delete company brand.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns = useMemo(() => {
    ...
    const canUpdate = hasPermission("company_brand_master", "update");
    const canDelete = hasPermission("company_brand_master", "delete");
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, background: "#f8fafc", fontFamily: "'Inter',sans-serif" }}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", width: "100%", margin: "0 auto", padding: "32px 30px" }}>
        ...
        <DataTable
          ...
          actionButton={
            hasPermission("company_brand_master", "write") ? (
