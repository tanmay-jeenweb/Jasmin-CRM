...
import { getStoreBrands, createStoreBrand, updateStoreBrand, deleteStoreBrand } from "../../api/storeBrandApi";
import DataTable from "../../components/DataTable";
import toast from "react-hot-toast";
import { usePermission } from "../../context/PermissionContext";

// ─── Store Brand Modal (Handles both Create and Edit) ─────────────────────────
function StoreBrandModal({ isOpen, row, onClose, onSave, saving }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!name.trim()) return;
    onSave(isEdit ? row.id : null, name.trim(), forCode);
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "20px 28px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(135deg,#6804a1,#52037e)" }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#fff" }}>{isEdit ? "Edit Store Brand" : "Create Store Brand"}</h2>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#d9e2ec" }}>{isEdit ? "Update store brand details" : "Add a new store brand to the system"}</p>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ padding: "24px 28px" }}>
            <div style={{ marginBottom: 18 }}>
              ...
              <input
                ...
                required
                placeholder="e.g. Jasmin, Jasmin Mini, Store Brand"
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <select
                value={forCode}
                onChange={(e) => setForCode(e.target.value)}
                style={{
                  width: "100%", boxSizing: "border-box", border: "1.5px solid #cbd5e1", borderRadius: 9,
                  padding: "11px 14px", fontSize: 15, outline: "none", color: "#1e293b", background: "#fff", cursor: "pointer"
                }}
                onFocus={e => e.target.style.borderColor = "#6804a1"}
                onBlur={e => e.target.style.borderColor = "#cbd5e1"}
              >
          ...
          <div style={{ padding: "16px 28px", borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "flex-end", gap: 12, background: "#fafafa" }}>
            ...
            <button
              ...
              {saving ? "Saving…" : isEdit ? "Save Changes" : "Create Store Brand"}
            </button>
          </div>
        </form>
      </div>
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function StoreBrandMaster() {
  ...
  const loadBrands = async () => {
    ...
    try {
      ...
      setError("Unable to load store brands. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (id, name, forCode) => {
    setSaving(true);
    try {
      if (id) {
        // Edit Mode
        await updateStoreBrand(id, { name, forCode });
        toast.success("Store brand updated successfully");
      } else {
        // Create Mode
        await createStoreBrand({ name, forCode });
      ...
      console.error("Failed to save store brand", err);
      toast.error(err?.response?.data?.message || "Unable to save store brand. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this store brand?")) return;
    setSaving(true);
    try {
      ...
      console.error("Failed to delete store brand", err);
      toast.error(err?.response?.data?.message || "Unable to delete store brand.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns = useMemo(() => {
    ...
    const canUpdate = hasPermission("store_brand_master", "update");
    const canDelete = hasPermission("store_brand_master", "delete");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, background: "#f8fafc", fontFamily: "'Inter',sans-serif" }}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", width: "100%", margin: "0 auto", padding: "32px 30px" }}>
        ...
        <DataTable
          tableId="store_brand_master"
          title="Store Brand Master"
          data={brands}
          columns={columns}
          loading={loading}
          searchPlaceholder="Search store brands..."
          actionButton={
            hasPermission("store_brand_master", "write") ? (
              <button
                onClick={() => {
                  setSelectedRow(null);
                  setIsModalOpen(true);
