...
export default function UserHome() {
    ...
    const canReadFranchise = isAdmin || hasPermission("in_process_franchise", "read");
    const canReadReminders = isAdmin || hasPermission("reminder_master", "read");
    ...
    return (
        <div className="flex flex-col min-h-screen bg-slate-50 font-sans text-slate-900">
            ...
            <main className="flex-1 w-full max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                ...
                {!canReadFranchise && !canReadReminders ? (
                    <div className="bg-white rounded-2xl border border-slate-200/80 p-8 shadow-sm text-center py-16">
                        <div className="w-16 h-16 bg-purple-50 rounded-full flex items-center justify-center mx-auto mb-4 border border-purple-100">
                            <i className="fa-solid fa-house-chimney-user text-purple-600 text-2xl"></i>
                        </div>
                        <h2 className="text-lg font-bold text-slate-800 mb-2">Welcome to Jasmin CRM</h2>
                        <p className="text-slate-500 text-sm max-w-md mx-auto leading-relaxed">
                            You are successfully logged in. Please use the navigation menu above to access available modules.
                        </p>
                    </div>
                ) : (
                    <div className={`grid grid-cols-1 ${canReadFranchise && canReadReminders ? "lg:grid-cols-3" : "lg:grid-cols-1"} gap-8 items-stretch`}>
